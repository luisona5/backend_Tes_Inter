...
const registrarDirector = async (req, res) => {
  try {
    ...
    // Crear el nuevo director con rol y referencia al administrador que lo crea
    
    const nuevoDirector = new Director({
      ...req.body,
            passwordDirector: await Director.prototype.encryptPassword("VET"+password),
            administrator: req.administratorHeader._id            
    });

    await nuevoDirector.save()
    await sendMailToOwner(emailDirector,"VET"+password)
